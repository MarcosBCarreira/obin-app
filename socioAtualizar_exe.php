	  ...
	  <!-- Acesso em:-->
		...
		<?php
			...
			if ($_FILES['Imagem']['size'] == 0) { // Não recebeu uma imagem binária
				$sql = "UPDATE Pessoas SET Nome = '$nome', CPF = '$CRM', Dt_Nasc = '$dtNasc' WHERE ID_Pessoa = $id";
			}else{
				$imagem = addslashes(file_get_contents($_FILES['Imagem']['tmp_name'])); // Prepara para salvar em BD
				$sql = "UPDATE Pessoas SET Nome = '$nome', CPF = '$CRM', Dt_Nasc = '$dtNasc', Foto = '$imagem' WHERE ID_Pessoa = $id";	
			}
